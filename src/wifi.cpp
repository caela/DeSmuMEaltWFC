...
*/
/* altWFC: this is the major file for us, compared to standard DeSmuME
the following commits were partially ignored:
* 4040 - 4044
* 4102 - 4105
* 4138
* 5346
...
FW_WFCProfile FW_WFCProfile1 = {"SoftAP",
								...
								0,
								{0, 0, 0, 0, 0, 0, 0},
								1, //altWFC: default to 1 in our case
...
/*altWFC: 10 was the value we've used successfully so far, let's stick with it:*/
#define WIFI_CMDCOUNT_SLICE 10
...
static void FillWFCProfile()
{
	//WIFI_LOG(2, "AltDNS1 set to: %s.\n", CommonSettings.wifi.altdns1);
	sscanf(CommonSettings.wifi.altdns1, "%hhd.%hhd.%hhd.%hhd", FW_WFCProfile1.PRIM_DNS, FW_WFCProfile1.PRIM_DNS + 1, FW_WFCProfile1.PRIM_DNS + 2, FW_WFCProfile1.PRIM_DNS + 3);
	sscanf(CommonSettings.wifi.altdns2, "%hhd.%hhd.%hhd.%hhd", FW_WFCProfile1.SEC_DNS, FW_WFCProfile1.SEC_DNS + 1, FW_WFCProfile1.SEC_DNS + 2, FW_WFCProfile1.SEC_DNS + 3);
	//Do we need some test here if we actually get a useful IP or something we don't want to like 0.0.0.0?
	WIFI_LOG(2, "AltDNS1 set to: %ld.%ld.%ld.%ld.\n", FW_WFCProfile1.PRIM_DNS[0], FW_WFCProfile1.PRIM_DNS[1], FW_WFCProfile1.PRIM_DNS[2], FW_WFCProfile1.PRIM_DNS[3]);
}


static void UpdateAltDNSinIni()
{
	char altdnsstr[20];
	
	sprintf(altdnsstr, "%ld.%ld.%ld.%ld", FW_WFCProfile1.PRIM_DNS[0], FW_WFCProfile1.PRIM_DNS[1], FW_WFCProfile1.PRIM_DNS[2], FW_WFCProfile1.PRIM_DNS[3]);

	if (strcmp(altdnsstr, CommonSettings.wifi.altdns1)) //AltDNS1 has been changed since FillWFCProfile
	{
		WritePrivateProfileString("Wifi", "AltDNS1", altdnsstr, IniName);
		WIFI_LOG(2, "AltDNS1 in Inifile set to: %s.\n", altdnsstr);
		sscanf(CommonSettings.wifi.altdns1, "%s", altdnsstr); //also update CommonSettings
	}

	sprintf(altdnsstr, "%ld.%ld.%ld.%ld", FW_WFCProfile1.SEC_DNS[0], FW_WFCProfile1.SEC_DNS[1], FW_WFCProfile1.SEC_DNS[2], FW_WFCProfile1.SEC_DNS[3]);

	if (strcmp(altdnsstr, CommonSettings.wifi.altdns2)) //AltDNS2 has been changed since FillWFCProfile
	{
		WritePrivateProfileString("Wifi", "AltDNS2", altdnsstr, IniName);
		WIFI_LOG(2, "AltDNS2 in Inifile set to: %s.\n", altdnsstr);
		sscanf(CommonSettings.wifi.altdns2, "%s", altdnsstr); //also update CommonSettings
	}
}
...
void WIFI_write16(u32 address, u16 val)
{
	...
	switch (address)
	{
		...
		case REG_WIFI_TXBUF_LOC3:
			wifiMac.TXSlot[(address - REG_WIFI_TXBUF_LOC1) >> 2] = val;
			WIFI_LOG(2, "Write to port %03X: %04X\n", address, val);
			break;
...
u16 WIFI_read16(u32 address)
{
	...
	switch (address)
	{
		...
		case REG_WIFI_TXBUF_LOC3:
			//printf("read TX reg %04X\n", address);
			return wifiMac.TXSlot[(address - REG_WIFI_TXBUF_LOC1) >> 2];
		case REG_WIFI_TXBUF_BEACON:
			//printf("read TX reg %04X\n", address);
			break;
		case REG_WIFI_EXTRACOUNTCNT:
	...
	case 0x2:				// Data frame
		{
			...
			if (((*(u16*)&packet[24]) == 0xAAAA) && ((*(u16*)&packet[26]) == 0x0003) && ((*(u16*)&packet[28]) == 0x0000))
			{
				...
				{
					//altWFC: we want to connect to AltWFC server
...
void WriteOutWFCProfileInfo(FW_WFCProfile *wfcprof)
{
	//char SSID[32];
	INFO("SSID set to: %s\n",wfcprof->SSID);
	//char SSID_WEP64[32];
	//INFO("SSID_WEP64 set to: %s\n",wfcprof->SSID_WEP64);
		/*
	...
	*/
	//u8 IP_ADDRESS[4];
	INFO("IP set to: %ld.%ld.%ld.%ld\n", wfcprof->IP_ADDRESS[0], wfcprof->IP_ADDRESS[1], wfcprof->IP_ADDRESS[2], wfcprof->IP_ADDRESS[3]);
	//u8 GATEWAY[4];
	INFO("Gatway set to: %ld.%ld.%ld.%ld\n", wfcprof->GATEWAY[0], wfcprof->GATEWAY[1], wfcprof->GATEWAY[2], wfcprof->GATEWAY[3]);
	//u8 PRIM_DNS[4];
	INFO("AltDNS1 set to: %ld.%ld.%ld.%ld\n", wfcprof->PRIM_DNS[0], wfcprof->PRIM_DNS[1], wfcprof->PRIM_DNS[2], wfcprof->PRIM_DNS[3]);
	//u8 SEC_DNS[4];
	INFO("AltDNS2 set to: %ld.%ld.%ld.%ld\n", wfcprof->SEC_DNS[0], wfcprof->SEC_DNS[1], wfcprof->SEC_DNS[2], wfcprof->SEC_DNS[3]);
	//u8 SUBNET_MASK; //need to write this differently? /24 means 255.255.255.0
	INFO("Subnet mask set to: /%ld\n", wfcprof->SUBNET_MASK);
	//u8 WEP64_KEY_AOSS[20];

	//u8 UNK1;
	INFO("?1 set to: %ld\n", wfcprof->UNK1);
	//u8 WEP_MODE;
	//INFO("WEP mode set to: %ld\n", wfcprof->WEP_MODE);
	//u8 STATUS;
	INFO("STATUS (0=Ready) set to: %ld\n", wfcprof->STATUS);
	//u8 UNK2[7];
	INFO("?2 set to: %02X-%02X-%02X-%02X-%02X-%02X\n", wfcprof->UNK2[0], wfcprof->UNK2[1], wfcprof->UNK2[2], wfcprof->UNK2[3], wfcprof->UNK2[4], wfcprof->UNK2[5], wfcprof->UNK2[6]);
	//u8 UNK3;
	INFO("?3 (??USE ALT DNS??) set to: %ld\n", wfcprof->UNK3);
	//u8 UNK4[14];
	INFO("?4_ set to:");
	for (int i = 0; i < 14; i++)
		INFO(" %02X",wfcprof->UNK4[i]);
	INFO("\n");
	//u8 CRC16[2]; 
	INFO("CRC16 set to: %lld\n", (u16*)wfcprof->CRC16[0]); //thinking right here?
}
