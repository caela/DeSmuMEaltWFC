...
bool NDS_FakeBoot()
{
	...
	/*altWFC: This was moved from main.cpp as part of commit 4665,
	* but this destroys any external firmware changes between
	* ROM resets (as long as we can not properly save those).
	* Btw, seems pointless really to create the internal firmware from
	* scratch at every single reset.
	* So moving it back to main.cpp */
...
void NDS_Reset()
{
	...
	/*altWFC: adding that line: writing WFC Profile from internal firmware
	*into the standard Profile, so we can fetch changed values for AltDNS
	*etc in WIFI_Reset.*/
	memcpy(&FW_WFCProfile1, (MMU.fw.data + 0x3FA40), sizeof(FW_WFCProfile));
	WIFI_Reset();
