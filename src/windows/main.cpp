...
//	struct configured_features my_config;
  ...
  video.height = 384;
	...
	/* altWFC: only reading from *.ini, not creating any config menues for this
	the user will change this in the game Nintendo Wifi settings,
	when that happens we should take the DNS settings and save as new default
	in the *.ini. The rest of the settings is more game specific? */
	GetPrivateProfileString("Wifi", "AltDNS1", DEFAULT_WFCSERVER_DNS, CommonSettings.wifi.altdns1, 20, IniName);
	GetPrivateProfileString("Wifi", "AltDNS2", DEFAULT_WFCSERVER_DNS, CommonSettings.wifi.altdns2, 20, IniName);
	...
	/* altWFC: moved back here from NDSSystem.cpp as it was before commit4665
	* We want the firmware created only once per DeSmuME session, not at every
	* ROM reload until we can properly save all firmware changes by the WFC */
